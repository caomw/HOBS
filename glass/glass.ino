/*
  ...
  This code is running on the glass, taking control of IR tranmission, XBee
  communication, and BT communication.

  XBee messages from client are routed to Google Glass for display purpose. BT
  messages from Google Glass either trigger IR transmission, or are being
  delivered to clients. As before, we are using XBee broadcast mechanism.

  Now we've switched to Arduino Mega, no need for software serial. For old code,
  using git log to find it...
...
// For Arduino Mega, we are using the hardware serial, which provides best
// performance
#define BT Serial1
#define XBee Serial2
...
void loop() {
  // in IR broadcast mode, we keep sending IR signal and hover the one with
  // largeste intensity reading
  if (ir_bcast_mode) {
    // constantly sending out ir broadcast as visual cue
    if(millis() - ir_time > ir_cycle) {
      // -1 (0xFFFF) indicates for broacast which is different than normal
      // -session id
    ...
    if (XBee.available()) {
      ...
      while (XBee.available()) {
				...
				message[2] = '\0';
				int id = atoi(message);
				intensity_array[id] = 0.1 * intensity_array[id] +
					0.9 * atoi(message+3);
