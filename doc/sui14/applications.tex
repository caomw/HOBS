...
\changes{Though our solution successfully solves the problem of {\em targets selection}, we haven't addressed the controlling issue when the device becomes complex. Users reported that \studyquote{for more complex appliances (TV), interaction was more difficult}. This problem can be partially ascribed to the limitations of Glass -- the touchpad can only detect a few gestures and the screen is too small. A separate work on interaction techniques for head-worn computing devices is needed to address these issues, while we keep our focus on the target selection in this paper.}
