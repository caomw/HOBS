...
\changes{After the map is created, the user can enter a quasi-mode for
  refinement by holding down on the touchpad. In this quasi-mode, one
  device lights up at a time. When the user turns their head in the
  direction of another device, the light switches to that
  device. Therefore, the user can move between devices one at a time
  with slight head movements. This prediction is implemented by
  calculating the user's direction of motion using a low-passed
  history of sensor measurements and searching through the adjacency
  map for the nearest device in that direction.}
...
\changes{Though we lack formal data to evaluate the performance of
this method, from our observeation during the experiments, we found
that each trial became much easier than previous iterations. We
conducted a survey to collect qualitative feedback after the
experiement.} On a scale from 1-7, 1 being the least mental effort and
