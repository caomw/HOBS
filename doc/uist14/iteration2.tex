...
Previously we only used IR reception as a binary signal for identifying potential targets. We hypothesize that IR intensity at the receiver side can provide more information about the likelihood that a user intended to select a particular target. Received IR intensity falls off with distance between IR emitter and receiver as well as with the angle between emitter and receiver. To measure intensity, we add \bjoern{add model number and characteristics} and replace the IR emitter with \bjoern{modelnumber, characteristics}.
We have empirically measured the intensity distribution at the receiver for this configuration in Figure~\ref{fig:measurement}. \ben{still working on it, not sure how much value we will get out of it. Seems useful to support the argument of the benefit though.}
Our measurements confirm that angular difference has a large effect on the intensity readings.
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{figures/placeholder.pdf}
\caption{placeholder for empirical measurement.}
...
\caption{The current environment setup \ben{needs annotation and a better photo.} \bjoern{yes}.}
...
In the cases when disambiguation is inevitable, {\em Intensity IR} sorts the list based on the intensity reading, while {\em Naive IR} sorts alphabetically. {\em Intensity IR} reduces the fraction of refinement trails in which additional list navigation is necessary (i.e., the first, already selected element is incorrect).  {\em Intensity IR} sorted the desired target as the first one in the list in 55\% of cases (93 of 167). In comparison, for {\em Naive IR}, only 35\% of trials sorted the desired target as the first one in the list (80 out of 219\bjoern{why isn't this 225 as above?}). Again, a Chi-square test show that this difference is significant (with $\chi^2(1) = 15.758$, $p < 0.0001$. 

Such gain can be seen from Figure~\ref{fig:study2} where the overall target acquisition time is obtained. \bjoern{avoid passive voice throughout!} The average selection time decreases from 4.31 seconds for {\em Naive IR} to 3.64 second for {\em Intensity IR}. This difference is also significant ($t(555)=3.2945$, $p=0.001$.) \bjoern{for consistency's sake, can we show a box plot as in figure 5?}
...
One side effect that we have observed in this approach is that the {\em Intensity IR} sometimes eliminates the desired target during the {\em scanning} stage. The this error rate (13 out of 300 cases, 4.3\%), is higher than the error rate for {\em Naive IR} \bjoern{insert error rate}. % but this is the trade-off for the overall faster acquisition time.
