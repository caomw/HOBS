...
refinement by holding down on the touchpad. In this quasi-mode, one device
lights up at a time. When the user turns his head in the direction of another
device, the light switches to that device. Therefore, the user can move between
devices one at a time with slight head movements. This prediction is
implemented by first calculating the user's direction of motion, and searching
...
Note that we use a hysteresis method to temporarily lock to the current device
...
experiment.} \achal{I vote changing to "Thoguh we lack... during the experiments,
we conducted..."} On a scale from 1-7, 1 being the least mental effort and
