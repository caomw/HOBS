...
Our augmentation to Google Glass adds a \bjoern{add modelnumber and spec details} LED (see Figure~\ref{fig:glass}. In the following description, we refer to Glass because the term is succinct; but  other suitable head-worn computing devices could be substituted.

\bjoern{need to mention that all the target nodes also have to be able to talk to each other and to glass outside the IR channel. We use XBees for this purpose.}
