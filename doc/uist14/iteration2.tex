...
We have emprically measured the intensity distribution at the receiver side, and it's shown in Figure~\ref{fig:measurement}. \ben{still working on it, not sure how much value we will get out of it. Seems useful to support the argument of the benefit though.}
...
From the measurement above we learned that the angluar difference has a large effect on the intensity readings. In order to make use of the intensity, we have designed and 3D printed a repositional IR holder that can be clipped onto Glass. Then for different users, they can adjust it to align with their line of sight. \ben{need to mention in iteration 1 about the crappy tape}. 
