...
In the within-subject study, half of the participants performed {\em IR selection} first and the other half used {\em list selection} first. For each selection condition, we conducted 15 target selections by randomly choosing from all the targets. During the study, we measured the {\bf target acquisition time} for each target selection. \changes{At the beginning of each acquisition, participants were asked to stand at a fixed position approximately 10-12 feet away from the nodes, looking down until the target is provided. As they attempt to connect, they may move around if they decided it would help them select. After each task, they were asked to move back to the starting position.} Afterward, participants were asked to complete a survey of primarily open-ended \changes{questions} about their experience.
...
As for the qualitative feedback, 11 out of \changes{16} participants preferred IR selection over list mode (\changes{3} preferred list, \changes{2} was undecided). \changes{When asked: \studyquote{When you have multiple candidate targets within sight, which method for selecting between them did you use?} (multiple choices), among 16 participates, 81\% chose {\em adjust head so that only one candidate is signaled}, 56\% chose {\em tap (to enter refinement stage) and then choose between list}, and 25\% chose {\em walk closer to the candidate for a better result}}. Although through our observation, many participants walked one or two steps during some tasks, this suggests that participants viewed their moving as a followed-through action of adjusting their heads instead of an intention to walk closer to the targets. 
