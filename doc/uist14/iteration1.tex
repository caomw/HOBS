...
Our results indicate that {\em IR selection} outperforms {\em list selection}.  The average target acquisition time for {\em IR selection} is 6.67 seconds \ben{one concern that I have is this is extremely long, 6 seconds. Do we need to explain more? One possible reason here is that we ask the user not to look at targets until we called out, so the visual search time is around 3 seconds. Should we say more or leaving 6 seconds there is fine?}, while {\em list selection} took 8.86 {\em list selection also requires visual search, I think...} seconds (see Figure~\ref{fig:ir_vs_list}). Student's t-test shows a significant difference ($t(279)=-3.81, p=0.00017$).
