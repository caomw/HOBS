...
\changes{IR signals are used for intial line-of-sight targeting; subsequently, we use bi-directions wireless communication to send IR signal status from targets to back to Glass and for device control itself. We have chosen a commercial off-the-shelf ZigBee implementation (XBee based on 802.15.4 radio) for this purpose (see Figure~\ref{fig:architecture}). This architecture was mostly chosen for reasons of expediency and we do not claim optimality for our design decisions. Future head-mounted devices could clearly integrate IR emitters; the choice of wireless technology could also change. In particular, one could substitute WiFi modules or design an all-Bluetooth network and forego intermediate layers in our design.}
