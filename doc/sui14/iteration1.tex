...
\changes{Most participants walked one or two steps during each task. In the survey we asked \studyquote{When you have multiple candidate targets within sight, which method for selecting between did you use?} (multiple choice), among 16 participates, X\% chose {\em adjust head so that only one candidate is signaled}, Y\% chose {\em walk closer to the candidate for a better result}, and only Z\% chose {\em tap (to enter refinement stage) and then choose between list}. \sean{will update numbers and add a few sentences here.}}
