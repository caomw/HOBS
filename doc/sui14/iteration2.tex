...
\caption{\changes{Empirical measurement of IR intensity at different positions. We measure the intensity on a horizontal plane at one-meter intervals from the IR emitter.} On that plane, a sample is taken every 5 centimeters, \changes{and we stop when the reading is smaller than ambient noise.} \changes{To match our assumption about the relationship between angles and intensities, the vertical axis is changed to the degree of the angle.} The size and color brightness represent the intensity of the readings for visualization.}
